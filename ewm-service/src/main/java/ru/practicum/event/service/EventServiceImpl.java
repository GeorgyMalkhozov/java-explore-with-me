...
public class EventServiceImpl implements EventService {
    ...
    public EventServiceImpl(EventRepository eventRepository, EventMapper eventMapper,
                            EventDao eventDao, ParticipationRequestService participationRequestService,
    ...
    public List<EventFullDto> getAllPublishedEventsByFilter(
        ...
        List<Event> events = entityManager.createQuery(cr).setFirstResult(from / size).setMaxResults(size)
                .getResultList();
        participationRequestService.setConfirmedRequestsCountToEvents(events);
    ...
    public EventFullDto updateEventByUser(Long eventId, Long userId, UpdateEventUserRequest dto) {
        ...
        eventStateUserActionProcessing(event, dto.getStateAction());
        eventDao.saveEvent(event);
        return eventMapper.eventToEventFullDto(event);
    }
    ...
    public EventFullDto updateEventByAdmin(Long eventId, UpdateEventAdminRequest dto) {
        ...
        eventStateAdminActionProcessing(event, dto.getStateAction());
        eventDao.saveEvent(event);
        return eventMapper.eventToEventFullDto(event);
    }
    ...
    private void eventStateUserActionProcessing(Event event, String eventStateAction) {
    ...
    private void eventStateAdminActionProcessing(Event event, String eventStateAction) {
        if (eventStateAction == null) {
            return;
        }
        eventDao.checkEventStateReadyForConfirmOrDecline(event);
